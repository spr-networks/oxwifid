...
# 2-link hostapd MLD AP over mac80211_hwsim. Derived from the hostap test suite's
# run_eht_mld_sae_two_links (test_eht.py): one MLO radio, hostapd global ctrl,
# two ADD <ifname> config= calls (link0 ch1 / link1 ch6 = one MLD AP), SAE+PMF.
#
# PASS = barely-cli prints PING_REPLY_OK and hostapd stops logging
# "not associated STA". Run on the hwsim box. hwsim radios only.
set -u
HOSTAPD=/home/ubuntu/hostap-hwsim/hostapd/hostapd
HCLI=/home/ubuntu/hostap-hwsim/hostapd/hostapd_cli
CLI=/home/ubuntu/barely-cli
G=/var/run/hostapd-mldg
W=/tmp/mldv
SSID=mldverify
PSK=qwertyuiop
...
cleanup() {
    sudo pkill -9 -f "$HOSTAPD .*$G" 2>/dev/null
    sudo pkill -9 -f "[h]ostapd-mldg" 2>/dev/null
...
sudo "$HOSTAPD" -g "$G" -B -t -f "$W/hapd.log"
sleep 1
echo "# ADD link0"; sudo "$HCLI" -g "$G" raw ADD "$AP_IF" config="$W/link0.conf"
sleep 1
echo "# ADD link1"; sudo "$HCLI" -g "$G" raw ADD "$AP_IF" config="$W/link1.conf"
sleep 2

# AP MLD + link0 addresses, for barely-cli.
AP_MLD=$(sudo "$HCLI" -p "$W/ctrl" -i "$AP_IF" status 2>/dev/null | sed -n 's/^mld_addr=//p' | head -1)
AP_L0=$(sudo "$HCLI" -p "$W/ctrl" -i "$AP_IF" status 2>/dev/null | sed -n 's/^link_addr\[0\]=//p; s/^link_addr=//p' | head -1)
...
[ -z "$AP_MLD" ] && { echo "FAIL: could not read AP mld_addr (AP may not have come up — see $W/hapd.log)"; sudo tail -5 "$W/hapd.log"; exit 1; }

echo "# running barely-cli MLD client (ping)…"
sudo timeout 30 "$CLI" --mode iface --iface "$CLI_IF" --channel 1 \
    --ssid "$SSID" --psk "$PSK" --sae \
...
echo "--- hostapd 'not associated' (should be empty after fix) ---"
sudo grep -a "not associated" "$W/hapd.log" | tail -3 || echo "(none)"
