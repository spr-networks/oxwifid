...
pkill -9 -f "hostap-hwsim" 2>/dev/null; pkill -9 -x wpa_supplicant 2>/dev/null; pkill -9 -x iperf3 2>/dev/null
...
sleep 5
grep -aq "START_AP ok" /tmp/iperf_ap.log && echo "AP up" >> "$R" || { echo "AP FAILED: $(tail -1 /tmp/iperf_ap.log)" >> "$R"; echo DONE >> "$R"; exit 1; }
...
echo DONE >> "$R"
pkill -9 -x iperf3 2>/dev/null; pkill -9 -f "[b]arely-ap --" 2>/dev/null; ip netns del "$NS" 2>/dev/null
