...
use barely_ap::sae::{self, Curve, Point, Sae, SaeError};
...
fn full_h2e_exchange_between_two_peers_agrees() {
    ...
    let ap_confirm = ap.write_confirm().unwrap();
    let sta_confirm = sta.write_confirm().unwrap();
...
#[test]
fn sae_confirm_rejects_duplicate_and_lower_authenticated_counters() {
    let mut verifier = Sae::new_h2e(b"replay-net", b"correct horse", None, &ADDR1, &ADDR2);
    let mut peer = Sae::new_h2e(b"replay-net", b"correct horse", None, &ADDR2, &ADDR1);
    verifier.prepare_commit(None);
    peer.prepare_commit(None);
    verifier
        .parse_peer_commit(&peer.write_commit())
        .expect("peer commit");
    peer.parse_peer_commit(&verifier.write_commit())
        .expect("verifier commit");
    verifier.process_commit().expect("verifier keys");
    peer.process_commit().expect("peer keys");

    let lower = peer.write_confirm().unwrap();
    let higher = peer.write_confirm().unwrap();

    // A forged high counter must not poison the receive window.
    let mut forged = higher.clone();
    forged[2] ^= 0x80;
    assert_eq!(verifier.check_confirm(&forged), Err(SaeError::BadConfirm));
    verifier
        .check_confirm(&higher)
        .expect("valid higher counter remains acceptable");

    assert_eq!(
        verifier.check_confirm(&higher),
        Err(SaeError::ReplayedConfirm),
        "an exact replay must be rejected"
    );
    assert_eq!(
        verifier.check_confirm(&lower),
        Err(SaeError::ReplayedConfirm),
        "a lower authenticated counter must be rejected"
    );
}

#[test]
fn parses_every_rsn_pmkid_in_wire_order() {
    let first = [0x11; 16];
    let second = [0x22; 16];
    let rsn = barely_ap::dot11::rsn_with_pmkid(&first);
    let mut body = rsn[2..].to_vec();
    body[20..22].copy_from_slice(&2u16.to_le_bytes());
    body.splice(38..38, second);

    assert_eq!(
        barely_ap::dot11::parse_rsn_pmkids(&body),
