...
pub struct EapolKey {
    ...
    pub key_nonce: [u8; 32],
    /// Authenticated group receive sequence counter (PN0..PN5, little-endian).
    pub key_rsc: u64,
    pub key_mic: [u8; 16],
    pub key_data: Vec<u8>,
...
impl EapolKey {
    ...
    pub fn parse(body: &[u8]) -> Option<EapolKey> {
        ...
        // This implementation negotiates RSN only. Descriptor type 254 belongs
        // to legacy WPA and must not enter the RSN state machine.
        if body[0] != 2 {
            return None;
        }
        ...
        let key_rsc = u64::from_le_bytes(body[61..69].try_into().ok()?);
        let mic_offset = 77;
