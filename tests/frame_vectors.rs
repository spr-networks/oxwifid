...
#[test]
fn eapol_requires_the_complete_llc_snap_header_and_rsn_descriptor() {
    let v = vectors();
    let raw = from_hex(v["frames"]["eapol_m2_incoming"]["bytes"].as_str().unwrap());
    let frame = dot11::Dot11::parse(dot11::strip_radiotap(&raw).unwrap()).unwrap();
    assert_eq!(&frame.body[..8], &[0xaa, 0xaa, 0x03, 0, 0, 0, 0x88, 0x8e]);

    for offset in 0..6 {
        let mut malformed = frame.clone();
        malformed.body[offset] ^= 0x01;
        assert!(
            !malformed.is_eapol(),
            "LLC/SNAP octet {offset} is security-significant"
        );
    }

    let mut legacy_descriptor = frame.eapol_key_body().unwrap().to_vec();
    legacy_descriptor[0] = 254;
    assert!(
        dot11::EapolKey::parse(&legacy_descriptor).is_none(),
        "legacy WPA descriptor type 254 must not enter the RSN state machine"
    );
}
