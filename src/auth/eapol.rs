...
pub fn build_eapol_m3_for_key_length(
    ...
    let body0 = build_eapol_key_body(ki, key_length, replay_counter, anonce, &[0u8; 16], &keydata);
    let mic = mic.compute(kck, &eapol_wrap(&body0));

    let body = build_eapol_key_body(ki, key_length, replay_counter, anonce, &mic, &keydata);
    let mut frame = eapol_data_header(bssid, sta, sc);
    frame.extend_from_slice(&eapol_wrap(&body));
    frame
}
